...
# parameters
# 1 epoch : 전체 데이터 셋을 1번 학습 시키는 것
training_epochs = 15
# 너무 많으니까 100개씩 읽어서 처리
batch_size = 100

with tf.Session() as sess:
    # Initialize TensorFlow variables
    sess.run(tf.global_variables_initializer())
    # Training cycle , Training Data를 사용
    for epoch in range(training_epochs):
        avg_cost = 0
        total_batch = int(mnist.train.num_examples / batch_size) # 몇 번 돌아야 될지 나옴
    ...
    # Test the model using test sets , Test Data를 사용
    print("Accuracy: ", accuracy.eval(session=sess, feed_dict={
          X: mnist.test.images, Y: mnist.test.labels}))

    # Get one and predict , Test Data를 사용
